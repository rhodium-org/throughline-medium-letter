# throughline-medium-letter

The **letter channel** of the **medium** content axis, expressed as a
...
how it is presented. This source is **only** the medium axis, and **only** its
**letter** channel: content posted or sent as a PDF, read linearly on paper, with no
links to click, no search and no scrolling, framed by an address block, a salutation
and a sign-off. It says nothing about:
...
The axis owns only what the channel imposes. That a letter *has* a sign-off is a
*medium* decision; the *register* of that sign-off — "Yours sincerely" versus a warmer
close — is a tone decision. The medium axis sits alongside the others and a consumer
composes all it needs.

Channels are **mutually exclusive**: a piece of writing is delivered as a letter *or* a
web page *or* an email *or* an SMS, never several at once. So each channel is a
**sibling** source (`throughline-medium-web`, and others as they are built) and a
consumer composes exactly one under the `medium` namespace — swapping channel is a
one-line `url`/`ref` change. A task like *"a plain, formal, general-reader letter"*
becomes a **compose** of `plain` + `tone-formal` + `audience-general` + `medium-letter`.
